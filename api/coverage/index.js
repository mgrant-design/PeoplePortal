/* api/coverage/index.js — read & write per-office coverage requirements in Cosmos.
   GET  /api/coverage[?office=]  → list saved coverage configs (optionally one office)
   POST /api/coverage            → upsert one office's coverage requirements

   A coverage doc holds the weekday and weekend role→count targets the scheduler uses
   to flag gaps and drive Smart fill. Security mirrors api/schedule: a valid Google ID
   token is required, the email is domain-locked, and (for writes) the caller must have
   scheduling rights derived from the live roster — never trusted from the client. */
...
/* ---- scheduling permission, derived from the roster (same port as api/schedule) ---- */
...
/* keep only non-negative integer counts for known-ish role keys */
function cleanReqs(obj) {
  const out = {};
  if (obj && typeof obj === 'object') {
    for (const [role, v] of Object.entries(obj)) {
      const n = Math.max(0, Math.min(99, Math.round(Number(v) || 0)));
      if (n > 0 && typeof role === 'string' && role.length <= 40) out[role] = n;
    }
  }
  return out;
}
...
module.exports = async function (context, req) {
  ...
  const coll = `dbs/${db}/colls/coverageReqs`;
  const strip = ({ _rid, _self, _etag, _attachments, _ts, ...rest }) => rest;

  try {
    /* ---------- READ: list saved coverage configs ---------- */
    if (req.method === 'GET') {
      ...
      context.res = { status: 200, headers, body: JSON.stringify({ coverage: docs }) };
      return;
    }

    /* ---------- WRITE: upsert one office's coverage requirements ---------- */
    if (req.method === 'POST') {
      ...
      if (!canSchedule(me, usersByEmail, managerEmails, employees)) {
        context.res = { status: 403, headers, body: JSON.stringify({ error: 'Not allowed to edit coverage requirements' }) };
        return;
      }
      ...
      if (!input || !input.office || typeof input.office !== 'string') {
        context.res = { status: 400, headers, body: JSON.stringify({ error: 'office is required' }) };
        return;
      }
      ...
      const doc = {
        id: input.office,
        office: input.office,
        weekday: cleanReqs(input.weekday),
        weekend: cleanReqs(input.weekend),
      ...
      context.res = { status: 200, headers, body: JSON.stringify({ ok: true, coverage: strip(up.body) }) };
      return;
    }
